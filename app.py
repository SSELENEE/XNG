...
# =========================
# NOAA SHAPEFILE PRIMARY ENGINE v0.9.5 STREAMLIT CLOUD SAFE
# =========================
...
# Approximate NCDC region polygons in lon/lat.
# This avoids geopandas/fiona/GDAL on Streamlit Community Cloud.
# It is primary-source NOAA shapefile + lightweight regional overlay.
NCDC_REGION_POLYGONS_LONLAT = {
    "Northwest": [(-125, 42), (-125, 49.5), (-111, 49.5), (-111, 42), (-117, 42), (-117, 42), (-125, 42)],
...
def load_lightweight_ncdc_regions_cached():
    if Polygon is None:
        raise RuntimeError("shapely is required. Run: pip install shapely")
...
def _read_noaa_records_pyshp(shp_path: Path):
    if shapefile is None or shapely_shape is None:
        raise RuntimeError("pyshp + shapely are required. Run: pip install pyshp shapely")
    ...
    for sr in reader.iterShapeRecords():
        ...
        out.append({"attrs": attrs, "geometry": geom})
    return out, fields

def analyze_noaa_temperature_shapefile(shp_path: Path, module: str) -> dict:
    """
    Streamlit Cloud safe NOAA engine for both NOAA 6-10D and NOAA 8-14D.

    Reads NOAA CPC shapefile attributes directly using pyshp:
    - Cat
    - Prob
    - geometry

    No geopandas / fiona / GDAL.
    """
    ...
    return {
        ...
        "analyzer": "v0.9.5 noaa shapefile primary pyshp/shapely",
        "generated_at": datetime.now().isoformat(),

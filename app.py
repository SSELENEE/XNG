...
# =========================
# NOAA SHAPEFILE PRIMARY ENGINE v0.9.4
# =========================
...
STATE_TO_NCDC_REGION = {
    "WA": "Northwest", "OR": "Northwest", "ID": "Northwest",
    "CA": "West", "NV": "West",
    "AZ": "Southwest", "NM": "Southwest", "UT": "Southwest", "CO": "Southwest",
    "MT": "West North Central", "WY": "West North Central", "ND": "West North Central", "SD": "West North Central", "NE": "West North Central", "KS": "West North Central",
    "MN": "East North Central", "IA": "East North Central", "MO": "East North Central", "WI": "East North Central", "IL": "East North Central", "MI": "East North Central", "IN": "East North Central", "OH": "East North Central",
    "KY": "Central", "TN": "Central", "AR": "Central", "LA": "Central", "MS": "Central", "AL": "Central",
    "OK": "South", "TX": "South",
    "VA": "Southeast", "WV": "Southeast", "NC": "Southeast", "SC": "Southeast", "GA": "Southeast", "FL": "Southeast",
    "ME": "Northeast", "VT": "Northeast", "NH": "Northeast", "MA": "Northeast", "RI": "Northeast", "CT": "Northeast", "NY": "Northeast", "PA": "Northeast", "NJ": "Northeast", "DE": "Northeast", "MD": "Northeast",
}
...
BASEMAP_COUNTIES_CANDIDATES = [
    "/opt/pyvenv/lib64/python3.13/site-packages/mpl_toolkits/basemap_data/UScounties.shp",
    "/opt/pyvenv/lib/python3.13/site-packages/mpl_toolkits/basemap_data/UScounties.shp",
]

def noaa_prob_to_score(cat, prob) -> float:
    """
    NOAA CPC shapefile fields:
    - Cat: Above / Below / Normal
    - Prob: probability band, usually 33, 40, 50, 60 ...

    For natural gas cooling-demand interpretation:
    - Above-normal temp = positive / bullish demand pressure
    - Below-normal temp = negative / bearish demand pressure
    - Normal = 0

    33% is baseline tercile confidence, not a strong signal.
    """
...
def find_basemap_counties_shp() -> Path:
    for p in BASEMAP_COUNTIES_CANDIDATES:
        fp = Path(p)
        if fp.exists():
            return fp
    raise FileNotFoundError(
        "UScounties.shp not found. Install basemap-data or update BASEMAP_COUNTIES_CANDIDATES."
    )

@st.cache_data(show_spinner=False)
def load_ncdc_region_geometries_cached() -> "gpd.GeoDataFrame":
    if gpd is None or unary_union is None:
        raise RuntimeError("geopandas/shapely are required for NOAA shapefile engine. Run: pip install geopandas shapely pyproj fiona")
    counties = gpd.read_file(find_basemap_counties_shp())
    counties = counties[counties["STATE"].isin(STATE_TO_NCDC_REGION.keys())].copy()
    counties["region"] = counties["STATE"].map(STATE_TO_NCDC_REGION)
    counties = counties.to_crs("EPSG:5070")
    rows = []
    for region, sub in counties.groupby("region"):
        rows.append({"region": region, "geometry": unary_union(list(sub.geometry))})
    return gpd.GeoDataFrame(rows, crs="EPSG:5070")

def save_uploaded_noaa_shapefile_set(day: str, module: str, uploaded_files: list) -> dict:
    """
    Save NOAA shapefile sidecars into a dedicated folder.
    Required minimum: .shp + .dbf + .shx.
    Recommended: .prj, .cpg, .sbn, .sbx, .shp.xml.
    """
    ...
    for up in uploaded_files:
        ...
        if out.suffix.lower() == ".shp":
            shp_rel = str(out.relative_to(ROOT))

    if not shp_rel:
        raise RuntimeError("No .shp file found. Upload the full NOAA shapefile set including .shp/.dbf/.shx.")

    return {"dir": str(outdir.relative_to(ROOT)), "shp_path": shp_rel, "files": saved}

def analyze_noaa_temperature_shapefile(shp_path: Path, module: str) -> dict:
    """
    Primary NOAA engine for both NOAA 6-10D and NOAA 8-14D.
    It reads NOAA CPC shapefile attributes directly: Cat / Prob / geometry.
    No screenshot, no crop, no color OCR, no warp.
    """
    if gpd is None or unary_union is None:
        raise RuntimeError("geopandas/shapely are required. Run: pip install geopandas shapely pyproj fiona")

    noaa = gpd.read_file(shp_path)
    required = {"Prob", "Cat", "geometry"}
    missing = required - set(noaa.columns)
    if missing:
        raise RuntimeError(f"NOAA shapefile missing fields: {missing}")

    if noaa.crs is None:
        noaa = noaa.set_crs("EPSG:4269")

    metadata = {}
    for col in ["Fcst_Date", "Start_Date", "End_Date"]:
        if col in noaa.columns and len(noaa):
            metadata[col] = str(noaa.iloc[0][col])

    noaa = noaa.to_crs("EPSG:5070").copy()
    noaa["cell_score"] = [noaa_prob_to_score(cat, prob) for cat, prob in zip(noaa["Cat"], noaa["Prob"])]

    regions = load_ncdc_region_geometries_cached()
    ...
    for _, reg in regions.iterrows():
        ...
        for _, cell in noaa.iterrows():
            ...
            if a <= 0:
                continue
            sc = float(cell["cell_score"])
            score_area_sum += sc * a
            area_sum += a
            contributors.append({
                "cat": str(cell["Cat"]),
                "prob": float(cell["Prob"]),
                "cell_score": round(sc, 4),
    ...
    records = sorted(records, key=lambda r: NOAA_NCDC_REGIONS.index(r["region"]))
    ...
    warm_regions = [r["region"] for r in records if r["raw_score"] >= 0.35]
    cool_regions = [r["region"] for r in records if r["raw_score"] <= -0.35]
    mixed_regions = [r["region"] for r in records if -0.35 < r["raw_score"] < 0.35]
    ...
    return {
        ...
        "summary": " ".join(parts),
        "reason": reason,
        "region_scores": {r["region"]: r["raw_score"] for r in records},
        "region_details": records,
        "composite_raw": round(composite, 4),
        "source_metadata": metadata,
        "analyzer": "v0.9.4 noaa shapefile primary",
        "generated_at": datetime.now().isoformat(),
        ...
        with left:
            if module_name == "CME NG Structure":
                ...
                if module_name.startswith("NOAA"):
                    ...
                    st.caption("Upload the full NOAA shapefile set: .shp + .dbf + .shx + .prj/.cpg/etc. This replaces screenshot OCR/warp.")
                    shp_files = st.file_uploader(
                        f"Drop NOAA shapefile files for {module_name}",
                        type=["shp", "dbf", "shx", "prj", "cpg", "sbn", "sbx", "xml"],
            ...
            if module_name.startswith("NOAA"):
                if st.button("Auto Fill from NOAA Shapefile", key=f"auto_{slugify(module_name)}", width="stretch"):
                    if not mod.get("source_path"):
                        st.warning("Upload the NOAA shapefile set first (.shp/.dbf/.shx).")
                    else:
                        try:
